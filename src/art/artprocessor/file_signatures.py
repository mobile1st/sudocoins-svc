...
def compare_sig(file_header, test_hex_string):
    if not file_header or len(file_header) == 0:
        return False
...
def type_from_sig(_header):
    for test_sig in signatures:
        if type(test_sig["hex"]) == list:
            for sig_hex in test_sig["hex"]:
                if compare_sig(_header, sig_hex):
                    return test_sig
        elif compare_sig(_header, test_sig["hex"]):
            return test_sig
    return None
