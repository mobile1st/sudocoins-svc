...
def get_day(collection_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            floor_points = '''select day(event_date), hour(event_date), min(price) from nft.events where event_date >= now() - interval 1 day and collection_id=%s group by day(event_date), hour(event_date) order by day(event_date) asc, hour(event_date) asc;'''
            ceiling_points = '''select day(event_date), hour(event_date), max(price) from nft.events where event_date >= now() - interval 1 day and collection_id=%s group by day(event_date), hour(event_date) order by day(event_date) asc, hour(event_date) asc;'''
            volume_points = '''select day(event_date), hour(event_date), sum(price) from nft.events where event_date >= now() - interval 1 day and collection_id=%s group by day(event_date), hour(event_date) order by day(event_date) asc, hour(event_date) asc;'''
            trades_points = '''select day(event_date), hour(event_date), count(*) from nft.events where event_date >= now() - interval 1 day and collection_id=%s group by day(event_date), hour(event_date);'''
            buyers_points = '''select day(event_date), hour(event_date), count(distinct buyer_id) from nft.events where event_date >= now() - interval 1 day and collection_id=%s group by day(event_date), hour(event_date);'''

            results = []
            statements = [floor, ceiling, volume, trades, buyers, avg_per_hour, floor_points, ceiling_points, volume_points, trades_points, buyers_points]
            for i in statements:
